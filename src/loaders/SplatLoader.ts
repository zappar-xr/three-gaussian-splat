...
export class SplatLoader extends THREE.Loader {
  ...
  public load(url: string, onLoad?: (data: Uint8Array) => void, onProgress?: (event: ProgressEvent) => void, onError?: (event: ErrorEvent) => void): void {
    ...
    fetch(path, {
      ...
      .then(buffer => {
        ...
        this.processData(data, data.length, true);
  ...
  public loadAsync(url: string, onProgress?: (event: ProgressEvent) => void): Promise<Uint8Array> {
    return new Promise((resolve, reject) => {
      this.load(url, resolve, onProgress, reject);
    });
  }

  private processData(data: Uint8Array, bytesRead: number, isComplete = false): void {
